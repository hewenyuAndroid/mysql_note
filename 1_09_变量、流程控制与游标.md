...
创建存储过程 `query_name_and_age_by_id`，在其内部定义 `emp_name`、`emp_age` 变量，在查询语句中赋值后，在存储过程内部调用查询变量的语句；

```sql
mysql> delimiter //
mysql> create procedure query_name_and_age_by_id(in emp_id int) begin declare emp_name varchar(50); declare emp_age int;select name, age into emp_name, emp_age from user where id = emp_id; select emp_name, emp_age; end //
Query OK, 0 rows affected (0.01 sec)

mysql> delimiter ;

# ============================================== 调用存储过程 ==============================================
mysql> call query_name_and_age_by_id(1);
+----------+---------+
| emp_name | emp_age |
+----------+---------+
| 张三     |      22 |
