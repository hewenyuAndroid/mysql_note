...
登录本机默认端口号(3306) mysql 服务: `mysql -u 用户名 -p 密码 ` // 注意，`-p` 后可以直接回车，使用密文登录

![mysql-login-default](./imgs/mysql-login-default.png)

## 2.2、登录本机，指定端口号

登录本机指定端口号 mysql 服务: `mysql -u 用户名 -P 端口号 -p`

![mysql-login-set-port](./imgs/mysql-login-set-port.png)

## 2.3、登录指定 `ip/port` 的 `mysql` 服务

登录指定ip的 mysql 服务: `mysql -u 用户名 -P 端口号 -h 主机ip -p` // 例如: 本机 ip 可以使用 `localhost` 或 `127.0.0.1`
...
| 目录                                      | 说明                                                                      |
| ----------------------------------------- | ------------------------------------------------------------------------- |
| `C:\Program Files\MySQL\MySQL Server 8.0` | `mysql` 的服务端和客户端的安装主目录 (包含可执行文件、库文件等)           |
...
## 3.1、`C:\ProgramData\MySQL\MySQL Server 8.0\Data` 数据存储目录 (`datadir`)
...
### 4.1.2、`create database 数据库名;` 创建数据库
...
### 4.1.4、`drop database 数据库名;` 删除数据库
...
### 4.1.5、`show create database 数据库名;` 查看数据库创建信息
...
### 4.2.2、`show tables from 数据库名;` 显示指定的数据库中的表;
...
### 4.2.6、`show create table 表名;` 查看数据库表创建信息
